...
class _PersonajeNPCState extends State<PersonajeNPC>
  ...
  void _iniciarCaminata() {
    ...
    setState(() {
      _estaCaminando = true;
      _anguloInclinacion =
          vaHaciaIzquierda ? -0.05 : 0.05; // Lo inclina hacia donde anda
    });
  ...
  Widget build(BuildContext context) {
    ...
    double anchoAvatar = anchoPantalla *
        0.35; // Ocupa un 35% de la pantalla para mantener proporción
...
class ForegroundMuroVias extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      const CustomPaint(painter: _MuroViasPainter());
}
...
class _MuroViasPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ESCALADO DINÁMICO: Si la pantalla es pequeña (móvil), reducimos el tamaño a la mitad
    double factorEscala = anchoLienzo < 600 ? 0.5 : 1.0;

    _dibujarTextoGraffiti(canvas, "FLUTTER", Offset(anchoLienzo * 0.05, 30),
        Colors.cyanAccent, 32 * factorEscala, -0.05);
    _dibujarTextoGraffiti(canvas, "SQL", Offset(anchoLienzo * 0.08, 190),
        Colors.cyan, 24 * factorEscala, 0.04);
    _dibujarTextoGraffiti(canvas, "LUCHADOR", Offset(anchoLienzo * 0.02, 110),
        Colors.red, 20 * factorEscala, 0.1);
  ...
  void _dibujarTextoGraffiti(Canvas lienzo, String texto, Offset posicion,
    ...
    final dibujanteTexto = TextPainter(
        text: TextSpan(
            ...
            style: TextStyle(
                ...
                fontSize: tamanoTexto, // Aquí recibe el tamaño ya escalado
    ...
    lienzo
        .restore(); // Devuelve el lienzo a su estado original (recto) para el próximo
  }

  @override
  bool shouldRepaint(_MuroViasPainter old) =>
      false; // Falso por eficiencia (es un muro estático)
}
